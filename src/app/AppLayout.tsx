...
const AppLayout: Component<ParentProps<TProps>> = (props) => {
  ...
  const nav = props.nav ?? (
    <p class="my-auto pl-2 text-rose-950 text-lg font-bold">Nav...</p>
  );
  ...
  return (
    <div class="flex flex-col bg-stone-700 h-screen">
      <AppLayoutHeader title={props.title} nav={nav}></AppLayoutHeader>
      <main class="flex flex-row">
        <div class="flex-1">{left}</div>
        <div class="w-[800px] bg-stone-500">{props.children}</div>
